...
function dumpCargo(world: GameState["world"], x: number, y: number, inv: Inventory, gold = 0) {
  ...
  if (origin && gold > 0) origin.goldDrop = (origin.goldDrop ?? 0) + gold;
  const items = ITEMS.filter((k) => (inv[k] ?? 0) > 0);
  if (items.length === 0) return;
  const cells: NonNullable<ReturnType<typeof tileAt>>[] = [];
  for (let r = 0; r <= 3; r++) {
  ...
  if (origin && !cells.includes(origin) && origin.biome !== "river" && origin.building !== "moat") cells.unshift(origin);
  for (const item of items) {
    const n = inv[item];
    let placed = false;
    for (const t of cells) {
      if (!t.pile) {
        t.pile = { item, amount: n };
        placed = true;
        break;
      }
      if (t.pile.item === item) {
    ...
    if (!placed && cells[0]) {
      const t = cells[0];
      t.chest = { ...chestOf(t), [item]: chestOf(t)[item] + n };
    }
  }
}
...
function speak(line: string, x: number, y: number, short?: string, tone: Floater["tone"] = "ok") {
  const s = useGame.getState();
  const prev = s.floaters ?? [];
  useGame.setState({
...
function walkTo(x: number, y: number) {
  ...
  if (isBusy(c)) {
    speak(`Занят: ${BUSY_LABEL[c.busy!.kind]}.`, c.x, c.y, BUSY_LABEL[c.busy!.kind], "bad");
    return;
  }
...
function busyBlock(): boolean {
  ...
  speak(
    `Занят: ${BUSY_LABEL[b.kind]}. Жди или брось сверху.`,
    s.character.x,
    s.character.y,
    BUSY_LABEL[b.kind],
    "bad",
...
export const useGame = create<GameState & Actions>((set, get) => ({
  ...
  clickTile: (x, y) => {
    ...
    if (isBusy(s.character) && !(s.character.x === x && s.character.y === y)) {
      speak(
        `Занят: ${BUSY_LABEL[s.character.busy!.kind]}. Жди или брось сверху.`,
        s.character.x,
        s.character.y,
        BUSY_LABEL[s.character.busy!.kind],
        "bad",
      );
      return;
    }
  ...
  setProfession: (p) => {
    const s = get();
    if (s.character.profWeek === s.week && s.character.profession !== "wanderer") {
      set({ log: pushLog(s.log, "Профессию на этой неделе уже менял. Завтрашняя неделя — пожалуйста.") });
      return;
    }
    set({
      character: { ...s.character, profession: p, profWeek: s.week },
      log: pushLog(s.log, `Профессия: ${PROFESSION_LABEL[p]}. ${PROF_BLURB[p]}`),
    });
  },
...
function catchUpSim(dt: number) {
  ...
  if (live.life === "jailed" && (live.jailedUntil ?? 0) > 0 && now >= live.jailedUntil) {
    ...
    useGame.setState({ log: pushLog(s.log, "Вышел. Розыск не сброшен.") });
...
function worldTick() {
  ...
  if (c.hp <= 0 && c.life === "alive") {
    ...
    if (c.wagon || c.transport === "wagon") {
      ...
      c.wagon = false;
      c.transport = c.horses > 0 ? "horse" : "walk";
    }
    c.life = "down";
    c.downAt = Date.now();
    c.hp = 0;
...
function gatherHere() {
  ...
  let got = Math.min(GATHER_YIELD[item] || 1, tile.amount);
  const match = PROFESSION_BIOME[s.character.profession]?.includes(tile.biome);
  if (match) got = Math.min(tile.amount, got + 1);
  if (s.character.hand === "axe" && item === "wood") got = Math.min(tile.amount, got + 1);
...
function pickLock(kind: "chest" | "gate") {
  ...
  const caught = Math.random() < Math.min(0.88, p + 0.16);
  ...
  if (caught) {
    ...
    c = applyCatch(c, law, `взлом у ${who}`);
    if (c.wagon || c.transport === "wagon") {
    ...
    useGame.setState({
      ...
      hint: { text: "Поймали. Замок цел.", tone: "bad" },
...
function pickupPile() {
  ...
  if ((!tile.pile || tile.pile.amount <= 0) && gold <= 0) return;
  const n = tile.pile?.amount ?? 0;
  const item = tile.pile?.item;
  const inv = item ? { ...s.character.inventory, [item]: s.character.inventory[item] + n } : { ...s.character.inventory };
  tile.pile = null;
  tile.goldDrop = 0;
  const bits: string[] = [];
  if (item && n) bits.push(`${n} ${ITEM_LABEL[item]}`);
...
function dropItem(item: ItemId, qty: number) {
  ...
  if (tile.pile && tile.pile.item !== item) {
    speak("Тут уже лежит другое. Подбери сначала.", tile.x, tile.y, "занято", "bad");
    return;
  }
  const inv = { ...s.character.inventory, [item]: have - n };
  tile.pile = { item, amount: (tile.pile?.amount ?? 0) + n };
  useGame.setState({
    character: { ...s.character, inventory: inv },
...
function buildOn(x: number, y: number, kind: BuildingKind) {
  ...
  } else if (!tile.owned && !tile.plot && kind !== "shack" && kind !== "field" && kind !== "pen" && kind !== "well") {
  ...
  tile.burned = false;
  const next = bumpSkill(
    { ...c, inventory: inv, gold: c.gold - cost.gold, energy: Math.max(0, c.energy - 2) },
    "build",
    0.2,
  );
  const spent: string[] = [];
  if (cost.wood) spent.push(`−${cost.wood} ${ITEM_LABEL.wood}`);
  if (cost.stone) spent.push(`−${cost.stone} ${ITEM_LABEL.stone}`);
  if (cost.gold) spent.push(`−${goldTxt(cost.gold)}`);
  useGame.setState({
    character: next,
    selected: { x, y },
    log: pushLog(s.log, `Поставил ${BUILDING_LABEL[kind]} на ${x},${y}.`),
...
function buildRoad(x: number, y: number, kind: "dirt" | "stone" | "bridge") {
  ...
  useGame.setState({
    character: { ...c, inventory: inv, energy: Math.max(0, c.energy - 2) },
    selected: { x, y },
    log: pushLog(s.log, `Положил ${label} на ${x},${y}.`),
...
function buyFromTrader(item: ItemId, qty: number) {
  ...
  let c = { ...s.character, inventory: inv, gold: s.character.gold - cost };
...
function bailOut() {
  ...
  if (s.character.gold < BAIL_GOLD) {
    speak(`Залог ${goldTxt(BAIL_GOLD)}.`, s.character.x, s.character.y, "мало золота", "bad");
    return;
  }
  useGame.setState({
...
function stealHere() {
  ...
  if (!loot) {
    if (tile.chestLock) {
      speak("Сундук на замке. Сначала взломай — или унеси с кучи у клетки.", tile.x, tile.y, "замок", "bad");
      return;
    }
    speak("Пусто. Уже вынесли.", tile.x, tile.y, "пусто", "bad");
    return;
  }
  const p = stealChance(s.world, tile, s.character, s.phase === "night");
  ...
  else if (tile.pile && tile.pile.item === loot.item) {
    tile.pile.amount -= loot.n;
    if (tile.pile.amount <= 0) tile.pile = null;
  ...
  if (caught) {
    const law = hasLaw(s.world, tile);
    c = applyCatch(c, law, `кража у ${who}`);
    if (c.wagon || c.transport === "wagon") {
      parkWagonNear(s.world, s.character.x, s.character.y, "you");
      c = { ...c, wagon: false, transport: c.horses > 0 ? "horse" : "walk" };
    }
    ...
    useGame.setState({
      ...
      log: pushLog(
        s.log,
        law
          ? `Поймали за кражу. Закон двора — яма. ${c.jailWhy}. Успел унести ${loot.n} ${ITEM_LABEL[loot.item]}.`
          : `Видели кражу. Закона нет — ямы нет, розыск. Унёс ${loot.n} ${ITEM_LABEL[loot.item]}.`,
      ),
      floaters: [...s.floaters, { id: ++floaterSeq, x: tile.x, y: tile.y, text: law ? "яма" : "видели", tone: "bad" as const }].slice(-10),
    });
    return;
...
function equipHand(item: ItemId | null) {
  ...
  useGame.setState({
    character: { ...s.character, hand: item },
...
function huntHere() {
  ...
  if (s.character.energy < 1) {
    speak("Нет сил на охоту.", tile.x, tile.y, "нет сил", "bad");
    return;
  }
  ...
  useGame.setState({
    character: {
      ...s.character,
      energy: Math.max(0, s.character.energy - 1),
      busy: makeBusy("hunt", tile.x, tile.y, Date.now() + ms),
      resting: false,
    },
    inspect: null,
    log: pushLog(s.log, `Охота на ${ANIMAL_LABEL[tile.herd.kind]} · ${Math.ceil(ms / 1000)} с. Копьё быстрее.`),
    floaters: [...s.floaters, { id: ++floaterSeq, x: tile.x, y: tile.y, text: `охота ${Math.ceil(ms / 1000)}с`, tone: "ok" as const }].slice(-10),
  });
  scheduleNotice("busy", "Охота кончилась", "Можно забирать добычу.", Date.now() + ms);
  useGame.getState().persist();
}
...
function catchHorse() {
  ...
  if (busyBlock()) return;
  const tile = hereTile();
  if (!tile?.herd || tile.herd.kind !== "horse" || !tile.herd.wild) {
    speak("Диких лошадей нет. Ищи табун на равнине.", s.character.x, s.character.y, "нет табуна", "bad");
    return;
  }
  if ((s.character.inventory.rope ?? 0) <= 0 && s.character.hand !== "rope") {
    speak("Нужна верёвка в руке. Плетётся из травы дома или в мастерской.", tile.x, tile.y, "нет верёвки", "bad");
    return;
  }
  if (s.character.energy < 1) {
    speak("Нет сил ловить.", tile.x, tile.y, "нет сил", "bad");
    return;
  }
...
function resolveHunt(s: GameState, c0: Character, tile: NonNullable<ReturnType<typeof tileAt>>) {
  ...
  tile.herd.count -= 1;
  if (tile.herd.count <= 0) tile.herd = null;
  const inv = { ...c0.inventory, food: c0.inventory.food + got };
...
function resolveCatch(s: GameState, c0: Character, tile: NonNullable<ReturnType<typeof tileAt>>) {
  if (!tile.herd || tile.herd.kind !== "horse" || !tile.herd.wild) {
    useGame.setState({ character: c0, log: pushLog(s.log, "Табуна уже нет.") });
    return;
  }
  const chance = 0.4 + c0.skills.agro * 0.04;
  if (Math.random() > chance) {
    tile.herd.count -= Math.random() < 0.3 ? 1 : 0;
    if (tile.herd.count <= 0) tile.herd = null;
    useGame.setState({
      character: c0,
      world: { ...s.world, tiles: s.world.tiles },
...
function resolveFish(s: GameState, c0: Character, tile: NonNullable<ReturnType<typeof tileAt>>) {
  ...
  const inv = { ...c0.inventory, fish: c0.inventory.fish + got };
  const c = bumpSkill({ ...c0, inventory: inv }, "survival", 0.12);
  useGame.setState({
    character: c,
    world: { ...s.world, tiles: s.world.tiles },
...
function burnHere() {
  ...
  if (tile.building !== "none" && !tile.burned) {
    ...
    if (dropN > 0) {
      ...
      chest[item] -= n;
      tile.chest = chest;
      tile.pile = { item, amount: (tile.pile && tile.pile.item === item ? tile.pile.amount : 0) + n };
    }
...
function fillBucket() {
  ...
  if ((s.character.inventory.bucket ?? 0) <= 0) {
    speak("Нужно ведро. Лепится из глины и дерева в мастерской.", tile.x, tile.y, "нет ведра", "bad");
    return;
  }
...
function doCraft(kind: CraftKind) {
  ...
  if (s.character.energy < needE) {
    speak(slow ? "Нет сил. Горн/печь без воды — дольше." : "Нет сил на ремесло.", tile!.x, tile!.y, "нет сил", "bad");
    return;
  }
  const inv = { ...s.character.inventory };
  ...
  useGame.setState({
    ...
    log: pushLog(
      s.log,
      def.who === "any"
        ? `Сделал: ${def.label} ×${def.n}.`
        : `${PROFESSION_LABEL[s.character.profession]}: ${def.label} ×${def.n}. Этого другие не делают.`,
    ),
...
function prospectHere() {
  ...
  useGame.setState({
    character: c,
    log: pushLog(s.log, hit ? "Нашёл кристалл." : "Пусто. Жила молчит."),
